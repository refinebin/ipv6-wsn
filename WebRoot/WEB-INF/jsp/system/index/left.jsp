<div id="sidebar" class="sidebar responsive">
	...
	<div style="display:none;" class="sidebar-shortcuts" id="sidebar-shortcuts">
		...
		<div style="display: none" class="sidebar-shortcuts-mini" id="sidebar-shortcuts-mini">
	...
	<ul class="nav nav-list">
		...
		<c:forEach items="${menuList}" var="menu1">
			<c:if test="${menu1.hasMenu && '1' == menu1.MENU_STATE}">
				<li class="leftList" id="lm${menu1.MENU_ID }">
								...
								<c:if test="${menu2.hasMenu && '1' == menu2.MENU_STATE}">
									<li class="" id="z${menu2.MENU_ID }">
										<a style="cursor:pointer;"
										   <c:if test="${not empty menu2.MENU_URL && '#' != menu2.MENU_URL}">target="mainFrame"
										   onclick="siMenu('z${menu2.MENU_ID }','lm${menu1.MENU_ID }','${menu2.MENU_NAME }','${menu2.MENU_URL }')"</c:if>
											...
											<i class="${menu2.MENU_ICON == null ? 'menu-icon fa fa-leaf black' : menu2.MENU_ICON}"></i>
												${menu2.MENU_NAME }
											<c:if test="${'[]' != menu2.subMenu}"><b
													class="arrow fa fa-angle-down"></b></c:if>
										</a>
										<b class="arrow"></b>
												...
												<c:forEach items="${menu2.subMenu}" var="menu3">
													<c:if test="${menu3.hasMenu && '1' == menu3.MENU_STATE}">
														<li class="" id="m${menu3.MENU_ID }">
															<a style="cursor:pointer;"
															   <c:if test="${not empty menu3.MENU_URL && '#' != menu3.MENU_URL}">target="mainFrame"
															   onclick="siMenu('m${menu3.MENU_ID }','z${menu2.MENU_ID }','${menu3.MENU_NAME }','${menu3.MENU_URL }')"</c:if>
																	...
																	<c:forEach items="${menu3.subMenu}" var="menu4">
																		<c:if test="${menu4.hasMenu && '1' == menu4.MENU_STATE}">
																			<li class="" id="n${menu4.MENU_ID }">
																				...
																				<a style="cursor:pointer;"
																				   target="mainFrame" target="mainFrame"
																				   onclick="siMenu('n${menu4.MENU_ID }','m${menu3.MENU_ID }','${menu4.MENU_NAME }','menu/otherlistMenu.do?MENU_ID=${menu4.MENU_ID }')">
																					</c:if>
																					<c:if test="${'[]' == menu4.subMenu}">
																					<a style="cursor:pointer;"
																					   target="mainFrame"
																					   <c:if test="${not empty menu4.MENU_URL && '#' != menu4.MENU_URL}">target="mainFrame"
																					   onclick="siMenu('n${menu4.MENU_ID }','m${menu3.MENU_ID }','${menu4.MENU_NAME }','${menu4.MENU_URL }')"</c:if>>
																						</c:if>
	...
	<div style="background-color: #7575FF" class="sidebar-toggle sidebar-collapse" id="sidebar-collapse">
		<i style="background-color: #FFFFFF" class="ace-icon fa fa-angle-double-left"
