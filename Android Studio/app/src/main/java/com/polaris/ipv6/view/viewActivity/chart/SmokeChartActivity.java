...
public class SmokeChartActivity extends AppCompatActivity {
    List<SensorRecordEntity> list;
    @BindView(R.id.SmokeLineChart)
    LineChart lineChart;
    MyLineChart myLineChart;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_smoke_chart);
        Intent intent = getIntent();
        list = (List<SensorRecordEntity>) intent.getSerializableExtra("SmokeList");
        ButterKnife.bind(this);
        myLineChart = new MyLineChart();
        myLineChart.initChart(lineChart);
        showLineChart(list, "空气质量", Color.CYAN);
    }
    /**
     * 展示曲线
     *
